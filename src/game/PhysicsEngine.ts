...
export class PhysicsEngine {
  ...
  constructor() {
    this.world = new CANNON.World({
      gravity: new CANNON.Vec3(0, -22, 0) // Crisp, snappy platformer gravity
    });
  ...
  private createGround() {
    ...
    // CANNON plane defaults to normal pointing in Z, rotate to point +Y
  ...
  public createPlayerBody(radius: number, height: number, spawnPos: THREE.Vector3): CANNON.Body {
    // Sphere body for smooth movement across steps and slopes
    const shape = new CANNON.Sphere(radius);
    const body = new CANNON.Body({
      mass: 65, // Human weight kg
      shape: shape,
      position: new CANNON.Vec3(spawnPos.x, spawnPos.y + radius, spawnPos.z),
      linearDamping: 0.1,
      fixedRotation: true // Prevent rolling like a ball
    });
    this.world.addBody(body);
    return body;
  }
